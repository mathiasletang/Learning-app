import { useMemo, useState } from 'react';
import { Link } from 'react-router-dom';
import { useLiveQuery } from 'dexie-react-hooks';
import { db } from '@/core/db';
import { toDayStr } from '@/core/date';
import { isMature } from '@/core/srs';
import { questionsByBank } from '@/core/content';
import { BANKS } from '@/core/meta';
import { cardInSubject, type SubjectDef } from '@/core/subjects';
import type { Flashcard, Question } from '@/core/types';
import {
  reviewFlashcard,
  createManualFlashcard,
  createFlashcardsFromQuestions,
  deleteFlashcard,
} from '@/app/actions';
import { useApp } from '@/app/store';
import { Button, Icon, Modal, Reveal } from '@/ui';
import { SrsReviewer, type ReviewItem, type GradeButton } from '@/modules/review/SrsReviewer';

const GRADES: GradeButton[] = [
  { grade: 0, label: 'Encore', tone: 'again' },
  { grade: 3, label: 'Difficile', tone: 'hard' },
  { grade: 4, label: 'Correct', tone: 'good' },
  { grade: 5, label: 'Facile', tone: 'easy' },
];

/**
 * Le circuit unique des erreurs de la matière : les questions ratées
 * deviennent des cartes, les cartes reviennent au bon moment, et l'on peut
 * aussi rejouer les ratées en conditions de QCM. Un seul endroit.
 */
export function SubjectRevision({ def }: { def: SubjectDef }) {
  const today = toDayStr();
  const pushToast = useApp((s) => s.pushToast);
  const allCards = useLiveQuery(() => db.flashcards.toArray(), [], []);
  const failedRows = useLiveQuery(
    () => db.qcmResults.where('status').equals(2).toArray(),
    [],
    [],
  );

  const [reviewing, setReviewing] = useState<Flashcard[] | null>(null);
  const [addOpen, setAddOpen] = useState(false);
  const [front, setFront] = useState('');
  const [back, setBack] = useState('');

  const cards = useMemo(() => allCards.filter((c) => cardInSubject(c, def.id)), [allCards, def.id]);
  const due = useMemo(() => cards.filter((c) => c.due <= today), [cards, today]);
  const mature = useMemo(() => cards.filter(isMature).length, [cards]);
  const byId = useMemo(() => new Map(cards.map((c) => [c.id, c])), [cards]);

  /* Questions ratées de la matière qui n'existent pas encore en carte. */
  const pending = useMemo<Question[]>(() => {
    const failedIds = new Set(failedRows.map((r) => r.qid));
    const existing = new Set(allCards.map((c) => c.id));
    const out: Question[] = [];
    for (const bank of def.banks) {
      for (const q of questionsByBank(bank)) {
        if (failedIds.has(q.id) && !existing.has(`fc:${q.id}`)) out.push(q);
      }
    }
    return out;
  }, [failedRows, allCards, def.banks]);

  /* Banques avec des ratées — pour rejouer en conditions de QCM. */
  const failedByBank = useMemo(() => {
    const failedIds = new Set(failedRows.map((r) => r.qid));
    return def.banks
      .map((b) => ({ bank: b, n: questionsByBank(b).filter((q) => failedIds.has(q.id)).length }))
      .filter((e) => e.n > 0);
  }, [failedRows, def.banks]);

  if (reviewing?.length) {
    const items: ReviewItem[] = reviewing.map((c) => ({
      id: c.id,
      front: <span dangerouslySetInnerHTML={{ __html: c.front }} />,
      back: (
        <div>
          <strong dangerouslySetInnerHTML={{ __html: c.back }} />
          {c.expl && <div className="card-face__expl" dangerouslySetInnerHTML={{ __html: c.expl }} />}
        </div>
      ),
    }));
    return (
      <SrsReviewer
        items={items}
        grades={GRADES}
        onGrade={async (item, g) => {
          const card = byId.get(item.id);
          if (card) await reviewFlashcard(card, g);
        }}
        onExit={() => setReviewing(null)}
      />
    );
  }

  async function integrate() {
    const n = await createFlashcardsFromQuestions(pending);
    pushToast({
      title: n > 0 ? `${n} carte${n > 1 ? 's' : ''} créée${n > 1 ? 's' : ''}` : 'Rien à intégrer',
      desc: n > 0 ? 'Vos erreurs rejoignent la file de révision.' : undefined,
    });
  }

  async function addCard() {
    if (!front.trim() || !back.trim()) return;
    // La carte libre est rattachée à la matière via sa première banque.
    await createManualFlashcard(front.trim(), back.trim(), def.id === 'cfa' ? 'cfa' : undefined);
    setFront('');
    setBack('');
    setAddOpen(false);
  }

  return (
    <>
      <div className="row row--wrap" style={{ gap: 'var(--s-3)', marginBottom: 'var(--s-10)' }}>
        {due.length > 0 ? (
          <Button variant="primary" icon="play" onClick={() => setReviewing(due)}>
            Réviser {due.length} carte{due.length > 1 ? 's' : ''}
          </Button>
        ) : (
          <span className="meta">Rien n'est dû aujourd'hui.</span>
        )}
        {pending.length > 0 && (
          <Button variant="secondary" icon="cards" onClick={integrate}>
            Intégrer {pending.length} erreur{pending.length > 1 ? 's' : ''} de QCM
          </Button>
        )}
        <span className="spacer" />
        <Button variant="ghost" icon="plus" onClick={() => setAddOpen(true)}>
          Nouvelle carte
        </Button>
      </div>

      <div className="figures">
        <div>
          <span className="figure__value tnum">{due.length}</span>
          <span className="eyebrow figure__label">Dues</span>
        </div>
        <div>
          <span className="figure__value tnum">{cards.length}</span>
          <span className="eyebrow figure__label">Cartes</span>
        </div>
        <div>
          <span className="figure__value tnum">{mature}</span>
          <span className="eyebrow figure__label">Mémorisées</span>
          <span className="micro figure__note">intervalle ≥ 21 jours</span>
        </div>
        <div>
          <span className="figure__value tnum">{failedByBank.reduce((n, e) => n + e.n, 0)}</span>
          <span className="eyebrow figure__label">Questions ratées</span>
        </div>
      </div>

      {failedByBank.length > 0 && (
        <section className="section">
          <div className="section__head">
            <h2>Rejouer en conditions de QCM</h2>
          </div>
          <div className="row row--wrap" style={{ gap: 'var(--s-3)' }}>
            {failedByBank.map((e) => (
              <Link key={e.bank} className="btn btn--secondary" to={`/qcm/${e.bank}?mode=review`}>
                {BANKS[e.bank].title} · {e.n}
              </Link>
            ))}
          </div>
        </section>
      )}

      <section className="section">
        <div className="section__head">
          <h2>Toutes les cartes</h2>
          <span className="meta tnum">{cards.length}</span>
        </div>
        {cards.length === 0 ? (
          <div className="empty">
            <h3>Aucune carte pour l'instant</h3>
            <p className="meta">
              Les questions manquées en séance de QCM deviennent des cartes, d'un seul geste.
            </p>
          </div>
        ) : (
          <div className="banklist">
            {cards.map((c, i) => (
              <Reveal key={c.id} delay={Math.min(i, 8) * 0.03} y={10}>
                <div className="bankrow" style={{ cursor: 'default' }}>
                  <span className="bankrow__index">{String(i + 1).padStart(2, '0')}</span>
                  <span style={{ minWidth: 0 }}>
                    <span
                      style={{ display: 'block', fontWeight: 500 }}
                      dangerouslySetInnerHTML={{ __html: c.front }}
                    />
                    <span
                      className="micro"
                      style={{ display: 'block', marginTop: 'var(--s-2)' }}
                      dangerouslySetInnerHTML={{ __html: c.back }}
                    />
                  </span>
                  <span className="meta tnum">{c.interval} j</span>
                  <button
                    type="button"
                    className="btn btn--ghost btn--icon"
                    aria-label="Supprimer la carte"
                    onClick={() => deleteFlashcard(c.id)}
                  >
                    <Icon name="trash" size={16} />
                  </button>
                </div>
              </Reveal>
            ))}
          </div>
        )}
      </section>

      <Modal
        open={addOpen}
        onClose={() => setAddOpen(false)}
        title="Nouvelle carte"
        footer={
          <>
            <span className="spacer" />
            <Button variant="ghost" onClick={() => setAddOpen(false)}>
              Annuler
            </Button>
            <Button variant="primary" onClick={addCard} disabled={!front.trim() || !back.trim()}>
              Créer
            </Button>
          </>
        }
      >
        <label className="stack" style={{ gap: 'var(--s-3)', marginBottom: 'var(--s-6)' }}>
          <span className="eyebrow">Recto — la question</span>
          <textarea className="field" rows={2} value={front} onChange={(e) => setFront(e.target.value)} />
        </label>
        <label className="stack" style={{ gap: 'var(--s-3)' }}>
          <span className="eyebrow">Verso — la réponse</span>
          <textarea className="field" rows={3} value={back} onChange={(e) => setBack(e.target.value)} />
        </label>
      </Modal>
    </>
  );
}
